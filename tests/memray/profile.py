...
def log(level=picologging.INFO):
    ...
    formatter = picologging.Formatter("%(created)f %(pathname)s:%(module)s:%(filename)s:%(lineno)d %(funcName)s %(levelno)d %(name)s - %(levelname)s %(process)d %(thread)d- %(message)s")
